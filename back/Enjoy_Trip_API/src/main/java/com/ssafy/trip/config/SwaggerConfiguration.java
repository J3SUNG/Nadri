...
public class SwaggerConfiguration {
	...
	private String title = "Enjoy Trip API " + version;
	...
	public Docket api() {
		return new Docket(DocumentationType.SWAGGER_2).consumes(getConsumeContentTypes())
				...
				.apis(RequestHandlerSelectors.basePackage("com.ssafy.trip.controller")).paths(regex("/.*")).build()
	...
	private ApiInfo apiInfo() {
		return new ApiInfoBuilder().title(title)
				.description("<h3>Enjoy Trip API Reference for Developers</h3>Swagger를 이용한 Enjoy Trip API<br>")
